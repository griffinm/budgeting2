class TagReportsController < ApplicationController
  ...
  # GET /api/tag_reports/:id
  def show
    @tag_report = current_user.account.tag_reports.includes(:tag_report_tags).find(params[:id])
  end
  ...
  # PATCH /api/tag_reports/:id
  def update
    @tag_report = current_user.account.tag_reports.find(params[:id])
    if @tag_report.update(tag_report_params)
      render :show
    else
      render json: { errors: @tag_report.errors.full_messages }, status: :bad_request
    end
